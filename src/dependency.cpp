...
//
template<typename Field>
int Depender(std::istream& input, QArray& Coeffs, const size_t maxnumcoeff,
...
// ============================================================
//
int DependerQ(std::istream& input, QArray& Coeffs, const size_t maxnumcoeff,
	     const size_t level, const Givaro::Integer& q) {
    if (Givaro::isZero(q)) {
	PLinOpt::QRat QQ;
	return PLinOpt::Depender(input, Coeffs, maxnumcoeff, level, QQ);
    } else {
	Givaro::Modular<Givaro::Integer> FF(q);
	return PLinOpt::Depender(input, Coeffs, maxnumcoeff, level, FF);
    }
}
...
int main(int argc, char** argv) {
    ...
    size_t level(4);
    PLinOpt::QArray Coeffs{1, -1};

    for (int i = 1; i<argc; ++i) {
	std::string args(argv[i]);
	if (args == "-h") {
	    std::clog << "Usage: " << argv[0]
		      << " [-h|[-c|-l|-q] #] [-v \"# ... #\"] [stdin|matfile.sms]\n"
		      << "  -c #: max number of coefficients per iteration\n"
		      << "  -v \"# ... #\": string of space separated coefficients\n"
	    ...
	    Coeffs.insert(Coeffs.end(), std::istream_iterator<Givaro::Rational>(sin),
			  std::istream_iterator<Givaro::Rational>());
	} else { filename = args; }
    }

    if (filename == "") {
	return PLinOpt::DependerQ(std::cin, Coeffs, maxnumcoeff, level, q);
    } else {
	std::ifstream inputmatrix(filename);
	return PLinOpt::DependerQ(inputmatrix, Coeffs, maxnumcoeff, level, q);
    }
